...
function network_probe_candidates($candidates, $candidateDevices, $maxSeconds)
{
    ...
    // 1. IPv4 All-Hosts Multicast, 3. mDNS, 4. SSDP
    $multicastTargets = array(
        array('ip' => '224.0.0.1', 'port' => 33434, 'payload' => "\x00"),
        array('ip' => '224.0.0.251', 'port' => 5353, 'payload' => "\x00\x00\x00\x00\x00\x01\x00\x00\x00\x00\x00\x00\x09_services\x07_dns-sd\x04_udp\x05local\x00\x00\x0c\x00\x01"),
        array('ip' => '239.255.255.250', 'port' => 1900, 'payload' => "M-SEARCH * HTTP/1.1\r\nHost: 239.255.255.250:1900\r\nMan: \"ssdp:discover\"\r\nST: ssdp:all\r\nMX: 1\r\n\r\n")
    );
    foreach ($multicastTargets as $target) {
        $fp = @stream_socket_client('udp://' . $target['ip'] . ':' . $target['port'], $errno, $errstr, 0, STREAM_CLIENT_ASYNC_CONNECT);
        if ($fp) { @stream_set_blocking($fp, false); @fwrite($fp, $target['payload']); $sockets[] = $fp; }
    }
    
    // 2. IPv6 All-Nodes Multicast (ff02::1) por interfaz
    $uniqueDevices = array_unique(array_filter(array_values($candidateDevices)));
    foreach ($uniqueDevices as $device) {
        if (!preg_match('/^[a-zA-Z0-9_.-]{1,16}$/D', $device)) continue;
        $fp = @stream_socket_client('udp://[ff02::1%' . $device . ']:33434', $errno, $errstr, 0, STREAM_CLIENT_ASYNC_CONNECT);
        if ($fp) { @stream_set_blocking($fp, false); @fwrite($fp, "\x00"); $sockets[] = $fp; }
    }
    foreach ($candidates as $candidate) {
        if (microtime(true) - $started >= $maxSeconds) {
            $timedOut = true;
            $probesAttempted = $probesAttempted > 0 ? null : 0;
            $probeReason = 'native_deadline';
            break;
        }
        $fp = @stream_socket_client('udp://' . $candidate . ':33434', $errno, $errstr, 0, STREAM_CLIENT_ASYNC_CONNECT);
        if ($fp) {
    ...
    // Breve pausa para enviar los paquetes.
    usleep(10000);
    foreach ($sockets as $fp) @fclose($fp);
    // Esperar respuestas ARP del kernel.
    usleep(400000);
    
    // Como el escáner nativo es ciego, usamos fping o ping solo para marcar quién responde a ICMP explícitamente.
    // Esto no anula el descubrimiento ARP que ya logramos arriba.
    if ($fping && $groups) {
        $method = 'fping-icmp';
        $completedGroups = 0;
...
function collectNetworkMap($active = false, $selectedNetworkId = null, &$rateReservation = null)
{
    ...
    if ($active) {
        ...
        if (!defined('PHP_VERSION_ID') || PHP_VERSION_ID < 70400 || (!network_find_binary(array('fping')) && !network_find_binary(array('ping')))) throw new RuntimeException('Instale fping o ping y use PHP 7.4 o posterior para habilitar el descubrimiento activo');
        ...
        if ($probe['method'] === 'unavailable') throw new RuntimeException('Instale fping o ping y use PHP 7.4 o posterior para habilitar el descubrimiento activo');
    ...
    <script nonce="<?php echo htmlspecialchars($cspNonce, ENT_QUOTES, 'UTF-8'); ?>">
        ...
        function networkHuman(value) {
            ...
            const labels = {
                ...
                local_interface: 'interfaz local', route_gateway: 'ruta de gateway', kernel_cache: 'caché del kernel', proc_arp: 'caché ARP', active_icmp: 'respuesta activa', ok: 'completa', partial: 'parcial', missing: 'ausente', denied: 'sin permiso', error: 'error', passive: 'pasivo', 'native-udp': 'escáner udp (nativo)', 'fping-icmp': 'fping · ICMP', 'ping-fallback': 'ping · fallback'
            };
        ...
        function networkReasonText(reason) {
            return ({
                disabled_by_policy: 'El descubrimiento ICMP/ARP está apagado. Para habilitarlo use PANEL_ENABLE_NETWORK_DISCOVERY=1 y una allowlist privada.',
                ...
                tool_missing: 'No hay una herramienta de red segura disponible; instale fping o ping.',
        ...
        async function runNetworkDiscovery() {
            ...
            const confirmed = window.confirm(`Se sondeará la red local (ARP/ICMP), sin escaneo de puertos ni DNS, dentro de ${scope.cidr} por ${scope.interface} (${scope.planned_count}/${scope.candidate_count} objetivos presupuestados). ¿Continuar?`);
            if (!confirmed) return;
            networkState.scanning = true;
            document.querySelectorAll('[data-network-discover]').forEach(button => { button.disabled = true; button.textContent = 'Sondeando…'; });
            networkState.request++;
            setNetworkStatus('Descubrimiento en curso dentro de las subredes autorizadas…', 'active');
            document.querySelectorAll('[data-network-refresh]').forEach(button => { button.disabled = true; });
            try {
        ...
        function renderNetworkTopology(data, canvas, details) {
            ...
            const visibleHosts = (data.hosts || []).filter(host => !host.is_self).sort((left, right) => Number(right.is_gateway) - Number(left.is_gateway) || Number(right.responded) - Number(left.responded)).slice(0, 64);
            const hostsByIface = new Map();
            visibleHosts.forEach(item => { const iface = (item.interfaces || []).find(name => positions.has(name)) || 'unknown'; if (!hostsByIface.has(iface)) hostsByIface.set(iface, []); hostsByIface.get(iface).push(item); });
            visibleHosts.forEach(item => {
                const iface = (item.interfaces || []).find(name => positions.has(name)) || 'unknown';
                const group = hostsByIface.get(iface); const indexInGroup = group.indexOf(item);
                const interfacePoint = positions.get(iface) || center;
                let angle = -Math.PI / 2;
                if (iface !== 'unknown' && interfaces.length > 0) {
                    const ifaceIndex = interfaces.findIndex(i => i.name === iface);
                    const baseAngle = (Math.PI * 2 * ifaceIndex / Math.max(1, interfaces.length)) - Math.PI / 2;
                    const spread = (Math.PI * 2) / Math.max(1, interfaces.length) * (interfaces.length === 1 ? 0.9 : 0.75);
                    angle = baseAngle + (group.length > 1 ? (indexInGroup / (group.length - 1)) * spread - (spread / 2) : 0);
                } else {
                    angle = (Math.PI * 2 * indexInGroup / Math.max(1, group.length)) - Math.PI / 2;
                }
                const stagger = indexInGroup % 2 === 0 ? 0 : 35;
                const point = { x: center.x + Math.cos(angle) * (400 + stagger), y: center.y + Math.sin(angle) * (255 + stagger * 0.7) };
        ...
        function renderNetworkView(data, connections = []) {
            ...
            const scopeLabel = makeElement('label', '', 'Subred autorizada para ARP/ICMP'); scopeLabel.htmlFor = 'networkScopeSelect';
